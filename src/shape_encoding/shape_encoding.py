...
from fvdb_utils import mesh_to_grid, pointcloud_to_grid
...
from laz_utils import load_laz_points_colors, estimate_normals_pca, orient_normals_toward_origin
...
class LazSampler:
    """Point cloud (.laz / .las) analogue of MeshSampler: NDC-normalized coords + PCA normals + optional LAS RGB."""
    ...
    def __init__(
        ...
        stride=1,
        max_points=None,
        n_neighbors_normals=30,
    ):
        xyz, rgb = load_laz_points_colors(
            laz_path, stride=stride, max_points=max_points
        )
        self.pts = (2 * mt.NDCnormalize(xyz)).astype(np.float32)
        self.colors_cloud = rgb
        normals = estimate_normals_pca(self.pts, n_neighbors=n_neighbors_normals)
        self.normals = orient_normals_toward_origin(self.pts, normals)

    def get_grid_and_samples(self, n=512, device="cuda", no_color=False):
        grid = pointcloud_to_grid(self.pts, n, device)
        grid_points = grid.grid_to_world(
            grid.ijk.double()).jdata.cpu().detach().numpy()
        nn = NearestNeighbors(n_neighbors=1, algorithm="auto").fit(self.pts)
        _, idx = nn.kneighbors(grid_points)
        idx = idx[:, 0]
        sampled_points = self.pts[idx]
        sampled_normals = self.normals[idx]
        if no_color or self.colors_cloud is None:
            sampled_colors = 0 * grid_points + 0.5
        else:
            sampled_colors = self.colors_cloud[idx]
        return grid, sampled_points, sampled_normals, sampled_colors


def process_GT_from_laz(
    name,
    laz_path,
    size=1024,
    targets=[512, 256, 128, 64, 32, 16],
    base_save_path="../../data/GT_sparse_tensors/",
    no_color=False,
    laz_stride=1,
    laz_max_points=None,
    normal_neighbors=30,
):
    print(f"Processing {name} from LAZ {laz_path}")
    ls = LazSampler(
        laz_path,
        stride=laz_stride,
        max_points=laz_max_points,
        n_neighbors_normals=normal_neighbors,
    )
    grid, sampled_points, sampled_normals, sampled_colors = ls.get_grid_and_samples(
        1024, "cpu", no_color
    )
    grid = grid.to("cuda")
    ...
    for t_size in targets:
        ...
        DT = DiffusionTensor.get_tensor_from_data(
            large_base_grid.grid,
            large_base_grid.normals,
            large_base_grid.local_offset,
            large_base_grid.colors,
            torch.ones_like(large_base_grid.local_offset[:, :1]),
        )
        size = t_size
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        default=None,
        type=str,
        help="Path to a .laz (LASzip) or .las point cloud; uses laspy with LASzip for .laz",
    )
    parser.add_argument(
        '-laz_stride',
        default=1,
        type=int,
        help="Keep every n-th point when reading LAZ/LAS (subsampling)",
    )
    parser.add_argument(
        '-laz_max_points',
        default=None,
        type=int,
        help="Random subsample to at most this many points after stride",
    )
    parser.add_argument(
        '-laz_normal_neighbors',
        default=30,
        type=int,
        help="k for PCA normal estimation on the point cloud",
    )
    ...
    try:
        ...
        pass
    if args.laz is not None:
        out_name = args.name if args.name is not None else Path(args.laz).stem
        process_GT_from_laz(
            out_name,
            args.laz,
            no_color=args.no_color,
            laz_stride=args.laz_stride,
            laz_max_points=args.laz_max_points,
            normal_neighbors=args.laz_normal_neighbors,
        )
